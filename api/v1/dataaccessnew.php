...
$app->post('/getgoaldatanew', function() use ($app) {
    ...
    for ($j=(int)$start_month;$j<=$end_month;$j++)
    {
        ...
        for ($i=1;$i<=$no_cols;$i++)
        {
            ...
            {
                $sql = "SELECT count(*) as goals_achieved FROM activity where (created_by = $user_id or modified_by = $user_id or FIND_IN_SET ($user_id ,assign_to)) and (month(created_date) = $j)  ".$year_query."  and (activity_type='Task') and (activity_sub_type='Lead generation')  ";
            }
            if ($goal_sub_category == "Other Activity")
            {
                $sql = "SELECT count(*) as goals_achieved FROM activity where (created_by = $user_id or modified_by = $user_id or FIND_IN_SET ($user_id ,assign_to)) and (month(created_date) = $j)  ".$year_query."  and (activity_type='Task') and (activity_sub_type='Other Activity')  ";
            }
...
$app->get('/userdashboard/:start_date/:end_date/:user_id', function($start_date,$end_date,$user_id) use ($app) {
    ...
    $mainsql = "SELECT * from goals as a LEFT JOIN dropdowns as b ON a.goal_sub_category = b.display_value WHERE a.user_id = $user_id and b.parent_type = a.goal_category and fy_year = '$this_year' ORDER BY CAST(b.sequence_number as UNSIGNED)";
    //------------------------------------------------------pks231023------------------------------------
    ...
    // error_log($mainsql, 3, "logfile7.log");
        ...
        while($row = $stmt->fetch_assoc())
        {
            ...
            if ($goals_to_achieve>0)
            {
                ...
                if ($goal_sub_category == "Transaction")
                {
                    ...
                    // error_log("pks01112023", 3, "logfile11.log");
                    // error_log($sql, 3, "logfile11.log");
                    $temp_sql .= $sql;
                ...
                if ($goal_sub_category == "Other Activity")
                {
                    $sql = "SELECT count(*) as goals_achieved FROM activity where (created_by = $user_id or modified_by = $user_id or FIND_IN_SET ($user_id ,assign_to)) and (created_date BETWEEN '$start_date' AND '$end_date') and (activity_type='Task') and (activity_sub_type='Other Activity') ";    
                }
                if ($goal_sub_category == "System Updation")
                {
    ...
    // error_log(print_r($goals_achieveddata, true), 3, "logfile7.log");
