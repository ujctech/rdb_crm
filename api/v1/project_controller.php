...
$app->post('/newsearch_projects', function() use ($app) {
    ...
    if (in_array("Admin", $role) || in_array("Sub Admin", $role))
    {  
        $searchsql = "SELECT a.*,e.company_name as developer,a.pack_price, CONCAT(a.salu,' ',a.fname,' ',a.lname) as project_contact,  (SELECT GROUP_CONCAT(CONCAT(k.salu,' ',k.fname,' ',k.lname) SEPARATOR ',') FROM users as j LEFT JOIN employee as k on k.emp_id = j.emp_id WHERE FIND_IN_SET(j.user_id , a.assign_to)) as assign_to, (SELECT GROUP_CONCAT(h.team_name SEPARATOR ',') FROM teams as h where FIND_IN_SET(h.team_id , a.teams)) as team , (SELECT count(*) from attachments as t where t.category_id = a.project_id and t.category = 'project') as project_image_count, a.add1,a.add2, DATE_FORMAT(a.created_date,'%d-%m-%Y %H:%i') AS created_date, DATE_FORMAT(a.modified_date,'%d-%m-%Y %H:%i') AS modified_date, concat(e.name_title,' ',e.f_name,' ',e.l_name) as developer_name, f.locality as locality, g.area_name as area_name,(SELECT filenames FROM attachments WHERE a.project_id = category_id and category='Project' and isdefault = 'true' LIMIT 1) as filenames, (SELECT CONCAT(max(pack_price),' ',any_value(pack_price_para)) FROM property WHERE project_id= a.project_id and pack_price > 0 ) as max_pack_price,(SELECT CONCAT(min(pack_price),' ',any_value(pack_price_para)) FROM property WHERE project_id= a.project_id and pack_price > 0 ) as min_pack_price , (SELECT CONCAT(max(CAST(carp_area as UNSIGNED)),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and CAST(carp_area as UNSIGNED) > 0 ) as max_carp,(SELECT CONCAT(min(CAST(carp_area as UNSIGNED)),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and CAST(carp_area as UNSIGNED) > 0 ) as min_carp,(SELECT CONCAT(u.salu,' ',u.fname,' ',u.lname) FROM users as s LEFT JOIN employee as u on u.emp_id = s.emp_id WHERE s.user_id = a.created_by) as created_by from project as a  LEFT JOIN contact as e on a.developer_id = e.contact_id  LEFT JOIN locality as f ON a.locality_id = f.locality_id LEFT JOIN areas as g ON a.area_id = g.area_id  WHERE a.created_by > 0 ";
    ...
    {
        $searchsql = "SELECT a.*,e.company_name as developer,a.pack_price, CONCAT(a.salu,' ',a.fname,' ',a.lname) as project_contact,  (SELECT GROUP_CONCAT(CONCAT(k.salu,' ',k.fname,' ',k.lname) SEPARATOR ',') FROM users as j LEFT JOIN employee as k on k.emp_id = j.emp_id WHERE FIND_IN_SET(j.user_id , a.assign_to)) as assign_to, (SELECT GROUP_CONCAT(h.team_name SEPARATOR ',') FROM teams as h where FIND_IN_SET(h.team_id , a.teams)) as team , (SELECT count(*) from attachments as t where t.category_id = a.project_id and t.category = 'project') as project_image_count,a.add1,a.add2, DATE_FORMAT(a.created_date,'%d-%m-%Y %H:%i') AS created_date, DATE_FORMAT(a.modified_date,'%d-%m-%Y %H:%i') AS modified_date, concat(e.name_title,' ',e.f_name,' ',e.l_name) as developer_name, f.locality as locality, g.area_name as area_name,(SELECT filenames FROM attachments WHERE a.project_id = category_id and category='Project' and isdefault = 'true' LIMIT 1) as filenames, (SELECT CONCAT(max(carp_area),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and carp_area > 0 ) as max_carp,(SELECT CONCAT(min(carp_area),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and carp_area > 0 ) as min_carp, (SELECT CONCAT(max(pack_price),' ',any_value(pack_price_para)) FROM property WHERE project_id= a.project_id and pack_price > 0 ) as max_pack_price,(SELECT CONCAT(min(pack_price),' ',any_value(pack_price_para)) FROM property WHERE project_id= a.project_id and pack_price > 0 ) as min_pack_price ,(SELECT CONCAT(u.salu,' ',u.fname,' ',u.lname) FROM users as s LEFT JOIN employee as u on u.emp_id = s.emp_id WHERE s.user_id = a.created_by) as created_by from project as a  LEFT JOIN contact as e on a.developer_id = e.contact_id   LEFT JOIN locality as f ON a.locality_id = f.locality_id LEFT JOIN areas as g ON a.area_id = g.area_id  WHERE (a.created_by = $user_id or FIND_IN_SET ($user_id ,a.assign_to) ".$team_query." ) ";
...
$app->get('/projectonemailerreports/:module_name/:id/:data', function($module_name,$id,$data) use ($app) {
    ...
    if (in_array("Admin", $role) || in_array("Sub Admin", $role))
    {
        ...
        $sql = "SELECT *,e.company_name as developer,SUBSTRING(e.f_name,1,1) as f_char,a.pack_price, CONCAT(a.salu,' ',a.fname,' ',a.lname) as project_contact,a.mob1,a.email, DATE_FORMAT(a.possession_date,'%d-%m-%Y') AS possession_date, (SELECT GROUP_CONCAT(CONCAT(k.salu,' ',k.fname,' ',k.lname) SEPARATOR ',') FROM users as j LEFT JOIN employee as k on k.emp_id = j.emp_id WHERE FIND_IN_SET(j.user_id , a.assign_to)) as assign_to, (SELECT GROUP_CONCAT(h.team_name SEPARATOR ',') FROM teams as h where FIND_IN_SET(h.team_id , a.teams)) as team , (SELECT count(*) from attachments as t where t.category_id = a.project_id and t.category = 'project') as project_image_count, a.add1,a.add2, DATE_FORMAT(a.created_date,'%d-%m-%Y %H:%i') AS created_date, DATE_FORMAT(a.modified_date,'%d-%m-%Y %H:%i') AS modified_date, concat(e.name_title,' ',e.f_name,' ',e.l_name) as developer_name, f.locality as locality, g.area_name as area_name,(SELECT CONCAT(max(exp_price),' ',any_value(exp_price_para)) FROM property WHERE project_id= a.project_id and exp_price > 0 ) as max_price,(SELECT CONCAT(min(exp_price),' ',any_value(exp_price_para)) FROM property WHERE project_id= a.project_id and exp_price > 0 ) as min_price , (SELECT CONCAT(max(CAST(carp_area as UNSIGNED)),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and carp_area > 0 ) as max_carp,(SELECT CONCAT(min(CAST(carp_area as UNSIGNED)),' ',any_value(carp_area_para)) FROM property WHERE project_id= a.project_id and carp_area > 0 ) as min_carp ,   (SELECT GROUP_CONCAT(DISTINCT(r.bedrooms) SEPARATOR ',') from project as q left join property as r on q.project_id = r.project_id where r.bedrooms is not null and r.bedrooms > 0 and r.project_id = a.project_id) as bedroom_list,(SELECT filenames FROM attachments WHERE a.project_id = category_id and category='Project' and isdefault = 'true' LIMIT 1) as filenames,(SELECT CONCAT(t.salu,' ',t.fname,' ',t.lname) FROM users as s LEFT JOIN employee as t on t.emp_id = s.emp_id WHERE s.user_id = a.created_by) as created_by FROM project as a  LEFT JOIN contact as e on a.developer_id = e.contact_id  LEFT JOIN locality as f ON a.locality_id = f.locality_id LEFT JOIN areas as g ON a.area_id = g.area_id WHERE a.project_id in ($data) GROUP BY a.project_id ";// CAST(a.project_id as UNSIGNED) DESC";
    }
