...
//pks new start -----------------------------------------------------------------------------14102023---------------------------------------------------------
// Expense Head

$app->get('/Expense_Head_Sub_Type_List_Ctrl', function() use ($app) {
    $sql  = "";
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    ...
    $sql = "SELECT expense_head_sub_type.*, expense_head.expense_title FROM expense_head_sub_type LEFT JOIN expense_head ON expense_head_sub_type.expense_head_id = expense_head.expense_head_id ORDER BY created_date DESC";
    $expense_heads = $db->getAllRecords($sql);
    echo json_encode($expense_heads);
});


$app->post('/expense_head_add_new_sub_type', function() use ($app) {
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    {
    ...
    $r->expense_headdata->created_date = $created_date;
        ...
        $tabble_name = "expense_head_sub_type";
        $column_names = array('expense_head_id','expense_sub_title','created_date','created_by');
        $multiple=array("");
        $result = $db->insertIntoTable($r->expense_headdata, $column_names, $tabble_name, $multiple);
        if ($result != NULL) {
            $response["status"] = "success";
            $response["message"] = "Expense Head Sub Type created successfully";
            $response["expense_head_id"] = $result;
            echoResponse(200, $response);
        } else {
            $response["status"] = "error";
            $response["message"] = "Failed to create Expense Head Sub type. Please try again";
            echoResponse(201, $response);
        }            
});


$app->get('/Expense_Head_Sub_Type_Edit_Ctrl/:expense_head_sub_type_id', function($expense_head_sub_type_id) use ($app) {
    $sql  = "";
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    ...
    $sql = "SELECT * from expense_head_sub_type where expense_sub_type_id=".$expense_head_sub_type_id;
    $expense_heads = $db->getAllRecords($sql);
    echo json_encode($expense_heads);
});

$app->post('/expense_head_update_sub_type', function() use ($app) {
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    {
    ...
    $modified_by = $session['user_id'];
    $modified_date = date('Y-m-d H:i:s');
    $expense_sub_type_id  = $r->expense_headdata->expense_sub_type_id;

    $r->expense_headdata->modified_by = $modified_by;
    $r->expense_headdata->modified_date = $modified_date;

        $tabble_name = "expense_head_sub_type";
        $column_names = array('expense_head_id', 'expense_sub_title', 'modified_by', 'modified_date');
        $condition = "expense_sub_type_id='$expense_sub_type_id'";
        $multiple = array("");
       
        $result = $db->updateIntoTable($r->expense_headdata, $column_names, $tabble_name,$condition);
        if ($result != NULL) {
            $response["status"] = "success";
            $response["message"] = "Expense Sub Type Updated successfully";
            echoResponse(200, $response);
        } else {
            $response["status"] = "error";
            $response["message"] = "Failed to Update Expense Sub Type. Please try again";
            echoResponse(201, $response);
        }
});

$app->post('/expense_head_delete_sub_type', function() use ($app) {
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    {
    ...
    $expense_sub_type_id  = $r->expense_headdata->expense_sub_type_id;
        ...
        $tabble_name = "expense_head_sub_type";
        $column_names = array('expense_sub_title');
        $condition = "expense_sub_type_id='$expense_sub_type_id'";
        $result = $db->deleteIntoTable($r->expense_headdata, $column_names, $tabble_name,$condition);
        if ($result != NULL) {
            $response["status"] = "success";
            $response["message"] = "Expense Sub type Deleted successfully";
            echoResponse(200, $response);
        } else {
            $response["status"] = "error";
            $response["message"] = "Failed to Delete Expense ub Type. Please try again";
            echoResponse(201, $response);
        }
});
//pks new end -----------------------------------------------------------------------------14102023---------------------------------------------------------
...
$app->post('/expense_add_new', function() use ($app) {
    ...
    $column_names = array('expense_type', 'expense_sub_type_id', 'expense_head_id', 'user_id', 'amount',  'expense_no', 'expense_date', 'comments', 'teams', 'sub_teams', 'payment_type', 'created_by', 'created_date');
...
$app->post('/expense_update', function() use ($app) {
    ...
    if($isexpenseExists){
        $tabble_name = "expense";
        $column_names = array('expense_type', 'expense_sub_type_id', 'expense_head_id', 'user_id', 'amount',  'expense_no', 'expense_date', 'comments', 'teams', 'sub_teams', 'payment_status', 'modified_by','modified_date');
        $multiple=array("");
...
$app->get('/select_expense_sub_types/:expense_head_id', function($expense_head_id) use ($app) {
    $sql  = "";
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    ...
    $sql = "SELECT * from expense_head_sub_type where expense_head_id = $expense_head_id";
    $expense_sub_types = $db->getAllRecords($sql);
    echo json_encode($expense_sub_types);
});
...
$app->get('/activity_list_ctrl/:cat/:id/:next_page_id', function($cat,$id,$next_page_id) use ($app) {
    ...
    if ($id == 0)
    {
        if (in_array("Admin", $role) || in_array("Sub Admin", $role))
        {
            ...
            $sql = "SELECT a.description,a.property_id,a.project_id,a.enquiry_id,a.client_id,a.developer_id,a.broker_id,a.assign_to,a.activity_id,a.activity_type,b.enquiry_for,b.enquiry_type, DATE_FORMAT(a.activity_start,'%d-%m-%Y %H:%i') AS activity_start, CONCAT(h.salu,' ',h.fname,' ', h.lname) as employee_name, CONCAT(c.name_title,' ',c.f_name,' ', c.l_name) as client, CONCAT(d.name_title,' ',d.f_name,' ', d.l_name) as developer, CONCAT(e.name_title,' ',e.f_name,' ', e.l_name) as broker,  DATE_FORMAT(a.created_date,'%d-%m-%Y %H:%i') AS created_date, DATE_FORMAT(a.modified_date,'%d-%m-%Y %H:%i') AS modified_date, DATE_FORMAT(i.start_date,'%d-%m-%Y %H:%i') AS start_date,a.status as status,a.activity_id as activity_id,(SELECT filenames FROM attachments WHERE g.emp_id = category_id and category='employee' LIMIT 1) as filenames from activity as a LEFT JOIN enquiry as b on a.enquiry_id = b.enquiry_id LEFT JOIN contact as c ON a.client_id = c.contact_id and c.contact_off = 'Client' LEFT JOIN contact as d on a.developer_id = d.contact_id and d.contact_off = 'Developer' LEFT JOIN contact e on a.broker_id = e.contact_id and e.contact_off = 'Broker' LEFT JOIN users as g on a.assign_to = g.user_id LEFT JOIN employee as h on h.emp_id = g.emp_id LEFT JOIN activity_details as i on a.activity_id = i.activity_id WHERE  a.created_date >= DATE_SUB(NOW(), INTERVAL 60 DAY)";
            
            $sql .=  " ORDER BY a.created_date DESC LIMIT $next_page_id, 30 "; // GROUP BY a.activity_id 

            $countsql = "SELECT count(*) as activity_count FROM activity as a WHERE  a.created_date >= DATE_SUB(NOW(), INTERVAL 60 DAY)";
...
$app->post('/activity_add_new', function() use ($app) {
    ...
    $r->activitydata->created_date = $created_date;
        //pks add 14092023
    // if($r->activitydata->activity_type == "Site Visit"){
    //     if(isset($r->activitydata->broker_id)){
    //         $r->activitydata->activity_type = "Broker Inspection";
    //     }else{
    //         $r->activitydata->activity_type = "Client Inspection";
    //     }
    // }
    if (isset($r->activitydata->activity_start))
    {
...
$app->post('/uploadenquiry', function() use ($app) {
    ...
    {   
        if($row == 1){
            ...
            $i++;
            $r->convertdata->enquiry_for=$objWorksheet->getCellByColumnAndRow(0, $row)->getValue();
            $r->convertdata->enquiry_type=$objWorksheet->getCellByColumnAndRow(1, $row)->getValue();
            $r->convertdata->enquiry_off=$objWorksheet->getCellByColumnAndRow(2, $row)->getValue();
            $r->convertdata->client=$objWorksheet->getCellByColumnAndRow(3, $row)->getValue();
            $r->convertdata->client_id=$objWorksheet->getCellByColumnAndRow(4, $row)->getValue();
            $r->convertdata->broker_involved=$objWorksheet->getCellByColumnAndRow(5, $row)->getValue();
            $r->convertdata->broker=$objWorksheet->getCellByColumnAndRow(6, $row)->getValue();
            $r->convertdata->con_status=$objWorksheet->getCellByColumnAndRow(7, $row)->getValue();
            $r->convertdata->loan_req=$objWorksheet->getCellByColumnAndRow(8, $row)->getValue();
            $r->convertdata->pre_leased=$objWorksheet->getCellByColumnAndRow(9, $row)->getValue();
            $r->convertdata->priority=$objWorksheet->getCellByColumnAndRow(10, $row)->getValue();
            
            // $r->convertdata->preferred_streets=$objWorksheet->getCellByColumnAndRow(11, $row)->getValue();
            $preferred_streets_get = $objWorksheet->getCellByColumnAndRow(11, $row)->getValue();
            $preferred_streets_get_array = explode(",", $preferred_streets_get);
            $p = array();
            foreach($preferred_streets_get_array as $b){
                $preferred_streets_sql = "SELECT * FROM `locality` WHERE `locality` = '$b'";
                $preferred_streetspresent = $db->getAllRecords($preferred_streets_sql);
                if(count($preferred_streetspresent) > 0){
                    $p[] = $preferred_streetspresent['0']['locality_id'];
                }
            }
            if(sizeof($p) > 0){
                $m = implode(',', $p);
                $r->convertdata->preferred_streets=$m;
            }
            else{
                $r->convertdata->preferred_streets= 0;
            }
            
            $r->convertdata->preferred_areas=$objWorksheet->getCellByColumnAndRow(12, $row)->getValue();
            $r->convertdata->preferred_city=$objWorksheet->getCellByColumnAndRow(13, $row)->getValue();
            $r->convertdata->preferred_state=$objWorksheet->getCellByColumnAndRow(14, $row)->getValue();
            $r->convertdata->preferred_country=$objWorksheet->getCellByColumnAndRow(15, $row)->getValue();
            $r->convertdata->bath=$objWorksheet->getCellByColumnAndRow(16, $row)->getValue();
            $r->convertdata->furniture=$objWorksheet->getCellByColumnAndRow(17, $row)->getValue();
            $r->convertdata->tot_area1=$objWorksheet->getCellByColumnAndRow(18, $row)->getValue();
            $r->convertdata->tot_area2=$objWorksheet->getCellByColumnAndRow(19, $row)->getValue();
            $r->convertdata->sale_area1=$objWorksheet->getCellByColumnAndRow(20, $row)->getValue();
            $r->convertdata->sale_area2=$objWorksheet->getCellByColumnAndRow(21, $row)->getValue();
            $r->convertdata->area_para=$objWorksheet->getCellByColumnAndRow(22, $row)->getValue();
            $r->convertdata->carp_area1=$objWorksheet->getCellByColumnAndRow(23, $row)->getValue();
            $r->convertdata->carp_area2=$objWorksheet->getCellByColumnAndRow(24, $row)->getValue();
            $r->convertdata->carp_area_para=$objWorksheet->getCellByColumnAndRow(25, $row)->getValue();
            $r->convertdata->budget_range1=$objWorksheet->getCellByColumnAndRow(26, $row)->getValue();
            $r->convertdata->budget_range1_para=$objWorksheet->getCellByColumnAndRow(27, $row)->getValue();
            ...
            $team_get = $objWorksheet->getCellByColumnAndRow(30, $row)->getValue();
            $team_get_array = explode(",", $team_get);
            $o = array();
            foreach($team_get_array as $b){
                $teamcheck_sql = "SELECT * FROM `teams` WHERE `team_name` = '$b'";
                $teamrowspresent = $db->getAllRecords($teamcheck_sql);
                if(count($teamrowspresent) > 0){
                    $o[] = $teamrowspresent['0']['team_id'];
                }
            }
            if(sizeof($o) > 0){
                $m = implode(',', $o);
                $r->convertdata->teams=$m;
            }
            else{
                $r->convertdata->teams= 0;
            }
            
            $sub_team_get = $objWorksheet->getCellByColumnAndRow(31, $row)->getValue();
            $sub_team_get_array = explode(",", $sub_team_get);
            $n = array();
            foreach($sub_team_get_array as $b){
                $sub_teamcheck_sql = "SELECT * FROM `sub_teams` WHERE `sub_team_name` = '$b'";
                $sub_teamrowspresent = $db->getAllRecords($sub_teamcheck_sql);
                if(count($sub_teamrowspresent) > 0){
                    $n[] = $sub_teamrowspresent['0']['sub_team_id'];
                }
            }
            if(sizeof($n) > 0){
                $m = implode(',', $n);
                $r->convertdata->sub_teams=$m;
            }
            else{
            ...
            $assign_to_get = $objWorksheet->getCellByColumnAndRow(32, $row)->getValue();
            ...
            if(sizeof($a) > 0){
                $f = implode(',', $a);
                $r->convertdata->assigned=$f;
            }
            else{
                $r->convertdata->assigned= 0;
            }
            ...
            $cellValuereg_date = $objWorksheet->getCellByColumnAndRow(44, $row)->getValue();
            ...
            $created_by_get = $objWorksheet->getCellByColumnAndRow(48, $row)->getValue();
                $name_datas = explode(' ', $created_by_get);
                $salu ="";
                $fname ="";
                $lname ="";
                if (isset($name_datas[0])) {
                    $salu = $name_datas[0];
                }
                if (isset($name_datas[1])) {
                    $fname = $name_datas[1];
                }
                if (isset($name_datas[2])) {
                    $lname = $name_datas[2];
                }
                $created_by_getcheck_sql = "SELECT * FROM `employee` WHERE `salu` = '$salu' AND `fname`= '$fname' AND `lname` = '$lname'";
                $created_by_getrowspresent = $db->getAllRecords($created_by_getcheck_sql);
                if(count($created_by_getrowspresent) > 0){
                    $emp_data = $created_by_getrowspresent[0]['emp_id'];
                    if($emp_data > 0){
                        $created_by_user_getcheck_sql = "SELECT * FROM `users` WHERE `emp_id` = '$emp_data'";
                        $created_by_user_getrowspresent = $db->getAllRecords($created_by_user_getcheck_sql);
                        if(count($created_by_user_getrowspresent) > 0){
                            $r->convertdata->created_by = $created_by_user_getrowspresent['0']['user_id'];
                        }else{
                            $r->convertdata->created_by= 0;
                        }
                    }
                }
            ...
            $pref_get = $objWorksheet->getCellByColumnAndRow(12, $row)->getValue();
            // $pref_get = $objWorksheet->getCellByColumnAndRow(50, $row)->getValue();
                $pref_get_array = explode(",", $pref_get);
                $k = array();
                foreach($pref_get_array as $b){
                    $pref_getcheck_sql = "SELECT * FROM `areas` WHERE `area_name` = '$b'";
                    $pref_getrowspresent = $db->getAllRecords($pref_getcheck_sql);
                    if(count($pref_getrowspresent) > 0){
                        $k[] = $pref_getrowspresent['0']['area_id'];
                    }
                }
                if(sizeof($k) > 0){
                    $m = implode(',', $k);
                    $r->convertdata->preferred_area_id=$m;
                }
                else{
                    $r->convertdata->preferred_area_id= 0;
                }
                //------------------=====================================
            
            $r->convertdata->sms_salestrainee=$objWorksheet->getCellByColumnAndRow(51, $row)->getValue();
            $r->convertdata->email_salestrainee=$objWorksheet->getCellByColumnAndRow(52, $row)->getValue();
            
            $cellValuecreated_date = $objWorksheet->getCellByColumnAndRow(53, $row)->getValue();
            ...
            // new add
            $r->convertdata->source_from=$objWorksheet->getCellByColumnAndRow(54, $row)->getValue();   //Source From
            
            // $r->convertdata->preferred_project_id=$objWorksheet->getCellByColumnAndRow(55, $row)->getValue();     //Preferred Project
            $pref_project_get = $objWorksheet->getCellByColumnAndRow(55, $row)->getValue();
            $pref_project_getcheck_sql = "SELECT * FROM `project` WHERE `project_name` = '$pref_project_get'";
            $pref_getrowspresentp = $db->getAllRecords($pref_project_getcheck_sql);
            // error_log(print_r($pref_getrowspresentp, true), 3, "logfile5.log");
            if(count($pref_getrowspresentp) > 0){
                $m = $pref_getrowspresentp[0]['project_id'];
                $r->convertdata->preferred_project_id=$m;
            }
            else{
                $r->convertdata->preferred_project_id= 0;
            }
            
            $r->convertdata->zip=$objWorksheet->getCellByColumnAndRow(56, $row)->getValue();     //Zip
            $r->convertdata->bedrooms=$objWorksheet->getCellByColumnAndRow(57, $row)->getValue();     //Bedrooms
            // new end
    
            $tabble_name = "enquiry";
            $column_names = array('enquiry_for','enquiry_type','enquiry_off','client','client_id','broker_involved','broker','con_status','loan_req','pre_leased','priority','preferred_streets','preferred_areas','preferred_city','preferred_state','preferred_country','bath','furniture','tot_area1','tot_area2','sale_area1','sale_area2','area_para','carp_area1','carp_area2','carp_area_para','budget_range1','budget_range1_para','budget_range2','budget_range2_para','teams','sub_teams','assigned','source_channel','subsource_channel','campaign','status','stage','vastu_comp','groups','intrnal_comment','external_comment','height','frontage','reg_date','parking','portal_id','published','created_by','broker_id', 'preferred_area_id','sms_salestrainee','email_salestrainee','created_date','source_from','preferred_project_id','zip','bedrooms');
            $multiple=array("");
            $result = $db->insertIntoTable($r->convertdata, $column_names, $tabble_name, $multiple);
    ...
    {
        $response["status"] = "success";
        $response["message"] = "$i enquries inserted successfully.";;
        echoResponse(201, $response);
    }
    else
    {
    ...
    {   
        if($row == 1){
            ...
            if($rowspresent->num_rows > 0){
                ...
                $locality_get = $objWorksheet->getCellByColumnAndRow(6, $row)->getValue();
                $localitycheck_sql = "SELECT * FROM `locality` WHERE `locality` = '$locality_get'";
                $locality_idrowspresent = $db->getAllRecords($localitycheck_sql);
                if(count($locality_idrowspresent) > 0){
                    $locality_id = $locality_idrowspresent[0]['locality_id'];
                    $r->convertdata->locality_id=$locality_id;
                }
                else{
                    $r->convertdata->locality_id= 0;
                }
                // $r->convertdata->area_id=$objWorksheet->getCellByColumnAndRow(7, $row)->getValue();
                $off_area_get = $objWorksheet->getCellByColumnAndRow(7, $row)->getValue();
                $off_areacheck_sql = "SELECT * FROM `areas` WHERE `area_name` = '$off_area_get'";
                $off_arearowspresent = $db->getAllRecords($off_areacheck_sql);
                if(count($off_arearowspresent) > 0){
                    $off_area_id = $off_arearowspresent[0]['area_id'];
                    $r->convertdata->area_id=$off_area_id;
                }
                else{
                    $r->convertdata->area_id= 0;
                }
            ...
            if(sizeof($n) > 0){
                ...
                $group_get = $objWorksheet->getCellByColumnAndRow(27, $row)->getValue();
                $group_get_array = explode(",", $group_get);
                $k = array();
                foreach($group_get_array as $b){
                    $groupcheck_sql = "SELECT * FROM `groups` WHERE `group_name` = '$b'";
                    $grouprowspresent = $db->getAllRecords($groupcheck_sql);
                    if(count($grouprowspresent) > 0){
                        $k[] = $grouprowspresent['0']['group_id'];
                    }
                }
                if(sizeof($k) > 0){
                    $m = implode(',', $k);
                    $r->convertdata->groups=$m;
                }
                else{
                    $r->convertdata->groups= 0;
                }
                ...
                $created_by_get = $objWorksheet->getCellByColumnAndRow(61, $row)->getValue();
                $name_datas = explode(' ', $created_by_get);
                $salu ="";
                $fname ="";
                $lname ="";
...
//pks new start 09102023
$app->post('/uploadactivity_files', function() use ($app) {
    session_start();
    $response = array();
    $sql  = "";
    $db = new DbHandler();
...
$app->post('/uploadactivity', function() use ($app) {
    $db = new DbHandler();
    $session = $db->getSession();
    if ($session['username']=="Guest")
    {
    ...
    {   
        if($row == 1){
                ...
                $i++;
                $r->convertdata->activity_type=$objWorksheet->getCellByColumnAndRow(0, $row)->getValue();
                $r->convertdata->activity_sub_type=$objWorksheet->getCellByColumnAndRow(1, $row)->getValue();
                
                $cellValueactivity_start = $objWorksheet->getCellByColumnAndRow(2, $row)->getValue();
                $excelDateactivity_start = ($cellValueactivity_start - 25569) * 86400;
                $formattedDateactivity_start = date("Y-m-d", $excelDateactivity_start); 
                $r->convertdata->activity_start=$formattedDateactivity_start;
                // $r->convertdata->activity_start=$objWorksheet->getCellByColumnAndRow(2, $row)->getValue();
                
                $cellValueactivity_end = $objWorksheet->getCellByColumnAndRow(3, $row)->getValue();
                $excelDateactivity_end = ($cellValueactivity_end - 25569) * 86400;
                $formattedDateactivity_end = date("Y-m-d", $excelDateactivity_end); 
                $r->convertdata->activity_end=$formattedDateactivity_end;
                // $r->convertdata->activity_end=$objWorksheet->getCellByColumnAndRow(3, $row)->getValue();
                $r->convertdata->description=$objWorksheet->getCellByColumnAndRow(4, $row)->getValue();
                $r->convertdata->enquiry_id=$objWorksheet->getCellByColumnAndRow(5, $row)->getValue();
                $r->convertdata->property_id=$objWorksheet->getCellByColumnAndRow(6, $row)->getValue();
                $r->convertdata->project_id=$objWorksheet->getCellByColumnAndRow(7, $row)->getValue();
                $r->convertdata->client_id=$objWorksheet->getCellByColumnAndRow(8, $row)->getValue();
                $r->convertdata->broker_id=$objWorksheet->getCellByColumnAndRow(9, $row)->getValue();
                $r->convertdata->developer_id=$objWorksheet->getCellByColumnAndRow(10, $row)->getValue();
                $r->convertdata->target_id=$objWorksheet->getCellByColumnAndRow(11, $row)->getValue();
                
                $team_get = $objWorksheet->getCellByColumnAndRow(12, $row)->getValue();
                $team_get_array = explode(",", $team_get);
                $o = array();
                foreach($team_get_array as $b){
                    $teamcheck_sql = "SELECT * FROM `teams` WHERE `team_name` = '$b'";
                ...
                $sub_team_get = $objWorksheet->getCellByColumnAndRow(13, $row)->getValue();
                $sub_team_get_array = explode(",", $sub_team_get);
                $n = array();
                foreach($sub_team_get_array as $b){
                    $sub_teamcheck_sql = "SELECT * FROM `sub_teams` WHERE `sub_team_name` = '$b'";
                ...
                $assign_to_get = $objWorksheet->getCellByColumnAndRow(14, $row)->getValue();
                $assign_to_get_array = explode(",", $assign_to_get);
                $a = array();
                foreach($assign_to_get_array as $b){
                    $name_data = explode(' ', $b);
                ...
                // $r->convertdata->teams=$objWorksheet->getCellByColumnAndRow(12, $row)->getValue();
                // $r->convertdata->sub_teams=$objWorksheet->getCellByColumnAndRow(13, $row)->getValue();
                // $r->convertdata->assign_to=$objWorksheet->getCellByColumnAndRow(14, $row)->getValue();
                $r->convertdata->remind=$objWorksheet->getCellByColumnAndRow(15, $row)->getValue();
                $r->convertdata->remind_before=$objWorksheet->getCellByColumnAndRow(16, $row)->getValue();
                $r->convertdata->remind_time=$objWorksheet->getCellByColumnAndRow(17, $row)->getValue();
                $r->convertdata->status=$objWorksheet->getCellByColumnAndRow(18, $row)->getValue();
                $r->convertdata->closure_comment=$objWorksheet->getCellByColumnAndRow(19, $row)->getValue();
                $r->convertdata->calling_count=$objWorksheet->getCellByColumnAndRow(20, $row)->getValue();
                
                $created_by_get = $objWorksheet->getCellByColumnAndRow(21, $row)->getValue();
                $name_datas = explode(' ', $created_by_get);
                $salu ="";
                $fname ="";
                $lname ="";
                ...
                // $r->convertdata->created_by=$objWorksheet->getCellByColumnAndRow(21, $row)->getValue();
                
                $cellValuecreated_date = $objWorksheet->getCellByColumnAndRow(22, $row)->getValue();
                $excelDatecreated_date = ($cellValuecreated_date - 25569) * 86400;
                $formattedDatecreated_date = date("Y-m-d", $excelDatecreated_date); 
                $r->convertdata->created_date=$formattedDatecreated_date;
                // $r->convertdata->created_date=$objWorksheet->getCellByColumnAndRow(22, $row)->getValue();
                
                $modified_by_get = $objWorksheet->getCellByColumnAndRow(23, $row)->getValue();
                $name_datas = explode(' ', $modified_by_get);
                $salu ="";
                $fname ="";
                $lname ="";
                if (isset($name_datas[0])) {
                ...
                $modified_by_getcheck_sql = "SELECT * FROM `employee` WHERE `salu` = '$salu' AND `fname`= '$fname' AND `lname` = '$lname'";
                $modified_by_getrowspresent = $db->getAllRecords($modified_by_getcheck_sql);
                if(count($modified_by_getrowspresent) > 0){
                    $emp_data = $modified_by_getrowspresent[0]['emp_id'];
                    if($emp_data > 0){
                        $modified_by_user_getcheck_sql = "SELECT * FROM `users` WHERE `emp_id` = '$emp_data'";
                        $modified_by_user_getrowspresent = $db->getAllRecords($modified_by_user_getcheck_sql);
                        if(count($modified_by_user_getrowspresent) > 0){
                            $r->convertdata->modified_by = $modified_by_user_getrowspresent['0']['user_id'];
                        }else{
                            $r->convertdata->modified_by= 0;
                        }
                    }
                }
                // $r->convertdata->modified_by=$objWorksheet->getCellByColumnAndRow(23, $row)->getValue();
                
                $cellValuemodified_date = $objWorksheet->getCellByColumnAndRow(24, $row)->getValue();
                $excelDatemodified_date = ($cellValuemodified_date - 25569) * 86400;
                $formattedDatemodified_date = date("Y-m-d", $excelDatemodified_date); 
                $r->convertdata->modified_date=$formattedDatemodified_date;
                // $r->convertdata->modified_date=$objWorksheet->getCellByColumnAndRow(24, $row)->getValue();
                
                $tabble_name = "activity";
                $column_names = array('activity_type', 'activity_sub_type', 'activity_start',  'activity_end', 'description', 'enquiry_id', 'property_id', 'project_id', 'client_id', 'broker_id', 'developer_id', 'target_id',
                                'teams', 'sub_teams', 'assign_to', 'remind', 'remind_before', 'status', 'closure_comment', 'calling_count', 'created_by', 'created_date', 'modified_by', 'modified_date');
        ...
        {
            $response["status"] = "success";
            $response["message"] = "$i activity inserted successfully.";
            echoResponse(201, $response);
        }
});
//pks new end 09102023
